...
def _save_to_github(data):
    ...
    if not token:
        ...
        return False
    api_url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/whitelisted_domains.json"
    headers = {
        'Authorization': f'token {token}',
        'Accept': 'application/vnd.github.v3+json',
    }
    for attempt in range(3):
        try:
            ...
            logger.warning("GitHub save attempt %d/3 failed for whitelist: %s", attempt + 1, e)
            if attempt < 2:
                time.sleep(1)
    logger.error("All 3 attempts to save whitelist failed")
    return False
